...
public class SimpleSound{
  ...
  public SimpleSound(int numFrames)
  {
    ...
    AudioTrack audio = new AudioTrack(AudioManager.STREAM_MUSIC,
            ...
            AudioFormat.ENCODING_PCM_16BIT, numFrames,
     ...
     */
    audioFileFormat = 
      new AudioFileFormat(AudioFileFormat.Type.WAV, 
                          audio, lengthInFrames);
  ...
  public SimpleSound(int sampleSizeInBits, boolean isBigEndian)
  {
    ...
    numFrames = (lengthInBytes/(numBytesInSample*numberOfChannels)) / 2;
  ...
  {
    if(audioFileFormat.getFormat().getChannelCount() == 1)
      return false;
    else
      return true;
  }
  ...
  public void loadFromFile(String inFileName) throws SoundException
  {
    ...
    else if(inFileName.toLowerCase().endsWith(".mp3"))
    {
      audioFileFormat = 
        new AudioFileFormat(AudioFileFormat.Type.MP3,
                            audioInputStream.getFormat(),
                            (int)audioInputStream.getFrameLength());
    }
  ...
  public void blockingPlayOld()
  {
    ...
    if (audio.getPlayState() == AudioTrack.PLAYSTATE_STOPPED || audio.getPlayState() == AudioTrack.PLAYSTATE_PAUSED)
    {
    	audio.play();
    }
  ...
  {   
    ...
    int newBufferSize = durInFrames * numFrames;
    
    byte[] newBuffer = new byte[newBufferSize];
    for(int i = 0; i <  newBufferSize; i++)
    {
      newBuffer[i] = oldBuffer[(startFrame*numFrames) + i];
    }
    ...
    AudioTrack newAF = 
      new AudioFormat(oldAFF.getFormat().getEncoding(),
                      ...
                      oldAFF.getFormat().getChannelCount(),
  ...
  public byte[] getFrame(int frameNum) throws SoundException
  {
    ...
    byte[] theFrame = new byte[numFrames];
    for (int i = 0; i < numFrames; i++)
    {
      theFrame[i] = buffer[frameNum*numFrames+i];
    }
    return theFrame;
  }
  ...
  public int getRightSample(int frameNum) throws SoundException
  {
    ...
    AudioFormat format = getAudioFileFormat().getFormat();
    int channels;
    if((channels = format.getChannelCount())==1)
    {
  ...
  {
    return getAudioFileFormat().getFormat().getChannelCount();
  }
  ...
  {
    AudioTrack track = getAudioFileFormat().getFormat();
    int sampleSizeInBits = track.getSampleSizeInBits();
    boolean isBigEndian = track.isBigEndian();
    
    byte[] theFrame = getFrame(frameNum);
    
    if(track.getEncoding().equals(AudioFormat.Encoding.PCM_SIGNED))
    {
